...
<div class="max-w-xl mx-auto bg-white rounded-xl shadow-md p-6 mt-8">
    <h1 class="text-2xl font-bold mb-6 text-gray-800">Modifier l'Année Académique</h1>
    ...
    <form method="POST" action="{{ route('annees_academiques.update', $anneeAcademique) }}">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label class="block text-gray-700 font-semibold mb-2">Nom de l'année académique</label>
            <input type="text" name="nom_annee" value="{{ old('nom_annee', $anneeAcademique->nom_annee) }}"
        ...
        <div class="mb-4">
            ...
            <input type="date" name="date_debut" value="{{ old('date_debut', $anneeAcademique->date_debut) }}"
                   class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
        </div>

        <div class="mb-4">
            <label class="block text-gray-700 font-semibold mb-2">Date de fin</label>
            <input type="date" name="date_fin" value="{{ old('date_fin', $anneeAcademique->date_fin) }}"
                   class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
        </div>

        <div class="mb-6">
            <label class="flex items-center">
                <input type="checkbox" name="est_actuelle" value="1"
                       {{ old('est_actuelle', $anneeAcademique->est_actuelle) ? 'checked' : '' }}
                       class="mr-2 rounded">
